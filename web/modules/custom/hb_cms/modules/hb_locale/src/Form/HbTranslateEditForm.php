...
namespace Drupal\hb_locale\Form;
 ...
 * @internal
 */
class HbTranslateEditForm extends TranslateEditForm {
	/**
	 * {@inheritdoc}
	 */
	public function buildForm(array $form, FormStateInterface $form_state) {
		...
		if (isset($langcode)) {
			...
			foreach ($strings as $string) {
				...
				$form['strings'][$string->lid]['is_deleted'] = [
					'#type' => 'checkbox',
				];
			}
	...
	public function submitForm(array &$form, FormStateInterface $form_state) {
		...
		foreach ($form_state->getValue('strings') as $lid => $new_translation) {
			...
			if ($new_translation['is_deleted']) {
				$this->localeStorage->findString(['lid' => $lid])->delete();
			}
		}
